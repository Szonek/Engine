...
public:
    ...
    void connect_to_server(const asio::ip::basic_resolver_results<asio::ip::tcp>& endpoints)
    {
        ...
        auto connection_parser = [this](std::error_code ec, asio::ip::tcp::endpoint ep)
        {
            if (!ec)
            {
        ...
        constexpr const bool wait_for_connection = true;
        if (wait_for_connection)
        {
            std::error_code ec;
            auto ep = asio::connect(socket_, endpoints, ec);
            connection_parser(ec, ep);
        }
        else
        {
            asio::async_connect(socket_, endpoints, connection_parser);
        }
    }
...
private:
    ...
    template<typename T>
    friend std::ostream& operator << (std::ostream& os, const Connection<T>& obj)
    {
        os << obj.socket_.remote_endpoint();
        return os;
    }
